...
import { Connection, QueryFilter, Model, SortOrder, Types } from 'mongoose';
...
export class AdminService {
  ...
  private buildUserQuery(filters: Pick<ListUsersQueryDto, 'search' | 'role' | 'status' | 'verified'>): QueryFilter<UserDocument> {
    ...
    // Accounts created before `isActive` existed have no value and count as active
    if (filters.status === 'active') query.isActive = { $ne: false };
    if (filters.status === 'suspended') query.isActive = false;
    if (filters.verified === 'verified') query.isEmailVerified = true;
    if (filters.verified === 'unverified') query.isEmailVerified = { $ne: true };
    return query;
  }

  async getAllUsers(params: ListUsersQueryDto) {
  ...
  async exportUsers(params: ListUsersQueryDto) {
  ...
  async updateUser(actorId: string, id: string, dto: AdminUpdateUserDto, suspendedReason?: string) {
    ...
    if (dto.role !== undefined && dto.role !== user.role) {
      if (isSelf) throw new ForbiddenException("You can't change your own role");
    ...
    if (dto.isActive !== undefined && dto.isActive !== wasActive) {
      if (isSelf) throw new ForbiddenException("You can't suspend your own account");
      if (!dto.isActive && user.role === 'admin') await this.assertNotLastAdmin();
      set.isActive = dto.isActive;
    }
    if (dto.isActive === true) unset.suspendedReason = 1;
    if (dto.isActive === false && suspendedReason?.trim()) set.suspendedReason = suspendedReason.trim();

    if (dto.isEmailVerified !== undefined) set.isEmailVerified = dto.isEmailVerified;
    if (dto.profileImage !== undefined) set.profileImage = dto.profileImage;
  ...
  async updateUserStatus(actorId: string, id: string, isActive: boolean, reason?: string) {
  ...
  async resetUserPassword(id: string, password: string) {
    ...
    await this.userModel.updateOne(
      { _id: user._id },
      { $set: { password: await bcrypt.hash(password, 10) }, $unset: { resetPasswordToken: 1, resetPasswordExpires: 1 } },
    );
    await this.notifyUser(
      user._id,
      'Your password was reset',
      'An administrator reset your password. If you did not expect this, contact support.',
      'warning',
    );
  ...
  async bulkUserAction(actorId: string, dto: BulkUserActionDto) {
    ...
    if (ids.length === 0) {
      throw new BadRequestException("You can't apply bulk actions to your own account");
    }
    const objectIds = ids.map((id) => new Types.ObjectId(id));
    const scope = { _id: { $in: objectIds } };

    // Actions that take admin rights away must leave at least one active admin behind
  ...
  private async notifyUser(userId: Types.ObjectId, title: string, message: string, type: 'info' | 'warning') {
    await this.notificationsService.create({ userId: userId.toString(), title, message, type }).catch(() => undefined);
  }

  /** Removes personal data owned by a deleted user. Reports and AI logs are kept for auditing. */
  private async removeUserData(userId: Types.ObjectId) {
